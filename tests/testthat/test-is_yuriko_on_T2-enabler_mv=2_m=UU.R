...
hand <- get_hand(testdata, c("Thassa's Oracle", "Mox Diamond", "Swamp", "Sink into Stupor"))
test_that("enabler 2 uu + mox diamond + 1 lands + u mdfc", 
          {expect_true(is_yuriko_on_T2(hand))})

hand <- get_hand(testdata, c("Thassa's Oracle", "Mox Diamond", "Swamp", "Boggart Trawler"))
test_that("enabler 2 uu + mox diamond + 1 land + mdfc but b mdfc", 
          {expect_false(is_yuriko_on_T2(hand))})

hand <- get_hand(testdata, c("Thassa's Oracle", "Mox Diamond", "Island", "Boggart Trawler"))
test_that("enabler 2 uu + mox diamond + 1 land + mdfc but b mdfc", 
          {expect_false(is_yuriko_on_T2(hand))})
