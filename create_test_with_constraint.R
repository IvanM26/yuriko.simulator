...
cards_that_must_be_present <- c("Dark Ritual", "Agadeem's Awakening", "Chrome Mox")

data_cards <- readxl::read_xlsx(filepath_decklist) %>% 
  dplyr::filter(card_name %in% cards_that_must_be_present)

n_random_cards <- 7 - length(cards_that_must_be_present)

decklist <- readxl::read_xlsx(filepath_decklist) %>% 
  dplyr::filter(in_deck == 1, !card_name %in% cards_that_must_be_present)
...
server <- function(input, output, session) {
  ...
  hand <- reactiveVal( bind_rows(data_cards, draw_seven(decklist)[1:6, ]) )
  ...
  observeEvent(input$keep, {
    ...
    hand( bind_rows(data_cards, draw_seven(decklist)[1:n_random_cards, ]) )
  })
