hand_produces_ub_with_constraint <- function(hand_metrics, constraint, use_lotus_petal = TRUE){
  ...
  if (hand_metrics$n_lands >= 2){
    ...
    if (hand_metrics$n_color_lands == 0){
      ...
      if (hand_metrics$n_mdfc_lands == 1){
        if (hand_metrics$mox_diamond) return(TRUE)
        if (use_lotus_petal & hand_metrics$lotus_petal & n_mdfc_lands_constrain_color >= 1) return(TRUE)
